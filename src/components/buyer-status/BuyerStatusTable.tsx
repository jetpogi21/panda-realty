...
import {
  BuyerStatusFormikInitialValues,
  BuyerStatusModel,
...
const BuyerStatusTable = <T,>({
  ...
  const pageParams = useModelPageParams<BuyerStatusSearchParams>(modelConfig);
  ...
  const useBuyerStatusSearchQuery = () =>
    useModelsQuery<T>(modelConfig, {
      ...queryParams,
      fetchCount: fetchCount.toString(),
    });

  const queryResponse = useBuyerStatusSearchQuery();
  ...
  */
  ...
  const rowActions = getBuyerStatusRowActions({
    currentData,
    setCurrentData,
  ...
  */
  ...
  const handleSubmit = async (values: BuyerStatusFormikInitialValues, formik: FormikHelpers<BuyerStatusFormikInitialValues>) => {
    //The reference is the index of the row
    const rowsToBeSubmitted = (
      values[
        pluralizedModelName as keyof BuyerStatusFormikInitialValues
      ] as BuyerStatusFormikInitialValues["BuyerStatus"]
    ...
    asyncUpdateRecords(payload).then((data) => {
      ...
      const { inserted, updated } =
        data as unknown as UpdateModelsData<BuyerStatusModel>;
      ...
      Object.keys(inserted).forEach((idx) => {
        ...
        formik.setFieldValue(`${pluralizedModelName}[${idx}]`, {
          ...
          touched: false,
          [primaryKeyFieldName]:
            inserted[numIdx][primaryKeyFieldName as keyof BuyerStatusModel],
        });
      });
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <BuyerStatusForm
            data={(row ? row : null) as BuyerStatusModel | null}
            id={
              row
  ...
  const commonProps = {
    ...
    rowActions,
    modelActions,
    SingleColumnComponent: BuyerStatusSingleColumn,
    requiredList,
    defaultFormValue,
    ...
    (modelConfig.isTable ? (
      ...
      <Formik
        initialValues={
          ...
          } as unknown as BuyerStatusFormikInitialValues
        }
...
export default BuyerStatusTable;
