...
const BuyerStatusFilterForm = <T,>({
  ...
  const { pathname, router, params, query } =
    useModelPageParams<BuyerStatusSearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: BuyerStatusFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<BuyerStatusFormikFilter>,
    formik: FormikHelpers<BuyerStatusFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<BuyerStatusSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<BuyerStatusFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as BuyerStatusFormikFilter);
      formik.submitForm();
    };
...
export default BuyerStatusFilterForm;
