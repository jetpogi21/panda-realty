...
import {
  PropertyStatusFormikInitialValues,
  PropertyStatusModel,
...
const PropertyStatusTable = <T,>({
  ...
  const pageParams =
    useModelPageParams<PropertyStatusSearchParams>(modelConfig);
  ...
  const usePropertyStatusSearchQuery = () =>
    useModelsQuery<T>(modelConfig, {
      ...queryParams,
      fetchCount: fetchCount.toString(),
    });

  const queryResponse = usePropertyStatusSearchQuery();
  ...
  */
  ...
  const rowActions = getPropertyStatusRowActions({
    currentData,
    setCurrentData,
  ...
  */
  ...
  const handleSubmit = async (
    values: PropertyStatusFormikInitialValues,
    formik: FormikHelpers<PropertyStatusFormikInitialValues>
  ) => {
    //The reference is the index of the row
    const rowsToBeSubmitted = (
      values[
        pluralizedModelName as keyof PropertyStatusFormikInitialValues
      ] as PropertyStatusFormikInitialValues["PropertyStatus"]
    ...
    asyncUpdateRecords(payload).then((data) => {
      ...
      const { inserted, updated } =
        data as unknown as UpdateModelsData<PropertyStatusModel>;
      ...
      Object.keys(inserted).forEach((idx) => {
        ...
        formik.setFieldValue(`${pluralizedModelName}[${idx}]`, {
          ...
          touched: false,
          [primaryKeyFieldName]:
            inserted[numIdx][primaryKeyFieldName as keyof PropertyStatusModel],
        });
      });
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <PropertyStatusForm
            data={(row ? row : null) as PropertyStatusModel | null}
            id={
              row
  ...
  const commonProps = {
    ...
    rowActions,
    modelActions,
    SingleColumnComponent: PropertyStatusSingleColumn,
    requiredList,
    defaultFormValue,
    ...
    (modelConfig.isTable ? (
      ...
      <Formik
        initialValues={
          ...
          } as unknown as PropertyStatusFormikInitialValues
        }
