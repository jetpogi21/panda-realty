...
const PropertyStatusFilterForm = <T,>({
  tableStates,
}: {
  ...
  const { pathname, router, params, query } =
    useModelPageParams<PropertyStatusSearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: PropertyStatusFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<PropertyStatusFormikFilter>,
    formik: FormikHelpers<PropertyStatusFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<PropertyStatusSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<PropertyStatusFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as PropertyStatusFormikFilter);
      formik.submitForm();
    };
...
export default PropertyStatusFilterForm;
