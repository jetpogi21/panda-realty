...
import {
  ContactCategoryFormikInitialValues,
...
const ContactCategoryTable = <T,>({
  ...
  const pageParams = useModelPageParams<ContactCategorySearchParams>(modelConfig);
  ...
  const useContactCategorySearchQuery = () =>
    useModelsQuery<T>(modelConfig, {
      ...queryParams,
      fetchCount: fetchCount.toString(),
    });

  const queryResponse = useContactCategorySearchQuery();
  ...
  */
  ...
  const rowActions = getContactCategoryRowActions({
    currentData,
    setCurrentData,
  ...
  */
  ...
  const handleSubmit = async (values: ContactCategoryFormikInitialValues, formik: FormikHelpers<ContactCategoryFormikInitialValues>) => {
    //The reference is the index of the row
    const rowsToBeSubmitted = (
      values[
        pluralizedModelName as keyof ContactCategoryFormikInitialValues
      ] as ContactCategoryFormikInitialValues["ContactCategories"]
    ...
    asyncUpdateRecords(payload).then((data) => {
      ...
      const { inserted, updated } =
        data as unknown as UpdateModelsData<ContactCategoryModel>;
      ...
      Object.keys(inserted).forEach((idx) => {
        ...
        formik.setFieldValue(`${pluralizedModelName}[${idx}]`, {
          ...
          touched: false,
          [primaryKeyFieldName]:
            inserted[numIdx][primaryKeyFieldName as keyof ContactCategoryModel],
        });
      });
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <ContactCategoryForm
            data={(row ? row : null) as ContactCategoryModel | null}
            id={
              row
  ...
  const commonProps = {
    ...
    rowActions,
    modelActions,
    SingleColumnComponent: ContactCategorySingleColumn,
    requiredList,
    defaultFormValue,
    ...
    (modelConfig.isTable ? (
      ...
      <Formik
        initialValues={
          ...
          } as unknown as ContactCategoryFormikInitialValues
        }
