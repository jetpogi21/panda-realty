...
const ContactCategoryFilterForm = <T,>({
  tableStates,
}: {
  ...
  const { pathname, router, params, query } =
    useModelPageParams<ContactCategorySearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: ContactCategoryFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<ContactCategoryFormikFilter>,
    formik: FormikHelpers<ContactCategoryFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<ContactCategorySearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<ContactCategoryFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as ContactCategoryFormikFilter);
      formik.submitForm();
    };
...
export default ContactCategoryFilterForm;
