...
const EntityCategoryFilterForm = <T,>({
  tableStates,
}: {
  ...
  const { pathname, router, params, query } =
    useModelPageParams<EntityCategorySearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: EntityCategoryFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<EntityCategoryFormikFilter>,
    formik: FormikHelpers<EntityCategoryFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<EntityCategorySearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<EntityCategoryFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as EntityCategoryFormikFilter);
      formik.submitForm();
    };
...
export default EntityCategoryFilterForm;
