...
const EntityFilterForm = <T,>({
  ...
  const { pathname, router, params, query } =
    useModelPageParams<EntitySearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: EntityFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<EntityFormikFilter>,
    formik: FormikHelpers<EntityFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
  ...
  const renderFormik = (formik: FormikProps<EntityFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as EntityFormikFilter);
      formik.submitForm();
    };
