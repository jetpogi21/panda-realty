...
const EntityTable = <T,>({
  ...
  const pageParams = useModelPageParams<EntitySearchParams>(modelConfig);
  ...
  const useEntitySearchQuery = () =>
    useModelsQuery<T>(modelConfig, {
      ...queryParams,
      fetchCount: fetchCount.toString(),
    });

  const queryResponse = useEntitySearchQuery();
  ...
  */
  ...
  const rowActions = getEntityRowActions({
    currentData,
    setCurrentData,
  ...
  */
  ...
  const handleSubmit = async (values: EntityFormikInitialValues, formik: FormikHelpers<EntityFormikInitialValues>) => {
    ...
    const rowsToBeSubmitted = (
      ...
      ] as EntityFormikInitialValues["Entities"]
    ...
    asyncUpdateRecords(payload).then((data) => {
      ...
      const { inserted, updated } =
        data as unknown as UpdateModelsData<EntityModel>;
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <EntityForm
            data={(row ? row : null) as EntityModel | null}
            id={
              row
  ...
  const commonProps = {
    ...
    rowActions,
    modelActions,
    SingleColumnComponent: EntitySingleColumn,
    requiredList,
    defaultFormValue,
    ...
    (modelConfig.isTable ? (
      ...
      <Formik
        initialValues={
          ...
          } as unknown as EntityFormikInitialValues
        }
