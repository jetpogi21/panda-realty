...
const PropertyFilterForm = <T,>({
  ...
  const { pathname, router, params, query } =
    useModelPageParams<PropertySearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: PropertyFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<PropertyFormikFilter>,
    formik: FormikHelpers<PropertyFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<PropertySearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<PropertyFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as PropertyFormikFilter);
      formik.submitForm();
    };
