...
const PropertyTable = <T,>({
  ...
  const pageParams = useModelPageParams<PropertySearchParams>(modelConfig);
  ...
  const usePropertySearchQuery = () =>
    useModelsQuery<T>(modelConfig, {
      ...queryParams,
      fetchCount: fetchCount.toString(),
    });

  const queryResponse = usePropertySearchQuery();
  ...
  */
  ...
  const rowActions = getPropertyRowActions({
    currentData,
    setCurrentData,
  ...
  */
  ...
  const handleSubmit = async (
    ...
    const rowsToBeSubmitted = (
      ...
      ] as PropertyFormikInitialValues["Properties"]
    ...
    asyncUpdateRecords(payload).then((data) => {
      ...
      const { inserted, updated } =
        data as unknown as UpdateModelsData<PropertyModel>;
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <PropertyForm
            data={(row ? row : null) as PropertyModel | null}
            id={
              row
  ...
  const commonProps = {
    ...
    rowActions,
    modelActions,
    SingleColumnComponent: PropertySingleColumn,
    requiredList,
    defaultFormValue,
    ...
    (modelConfig.isTable ? (
      ...
      <Formik
        initialValues={
          ...
          } as unknown as PropertyFormikInitialValues
        }
