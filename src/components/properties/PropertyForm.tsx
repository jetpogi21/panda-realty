...
"use client";
import {
  PropertyFormFormikInitialValues,
  PropertyModel,
...
interface PropertyFormProps {
  data: PropertyModel | null;
  id: string;
  modalFormProps?: ModelFormProps;
  hiddenField?: string;
  defaultValues?: Record<string, unknown>;
  onSubmit?: (
    values: PropertyFormFormikInitialValues,
    formik: FormikHelpers<PropertyFormFormikInitialValues>
  ) => void;
}
...
const PropertyForm: React.FC<PropertyFormProps> = (prop) => {
  const { id, modalFormProps, hiddenField, onSubmit, defaultValues } = prop;
  const { router, query, pathname } = useURL<PropertySearchParams>();
  ...
  if (property) {
    //@ts-ignore
    initialValues["index"] = property["index"];
  }
  ...
  const handleFormikSubmit = (
    ...
    /*
    if (hasUpdate) {
      const additionalValidationResponse = validateProperty(values);
    ...
    */
    ...
    //e.g. { deletedPropertyTags: [], newTags: []}
  ...
  const renderFormik = (
    formik: FormikProps<PropertyFormFormikInitialValues>
  ) => {
