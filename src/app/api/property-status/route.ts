...
export const GET = async (req: Request) => {
  ...
  const query = parseParams(searchParams) as Partial<PropertyStatusSearchParams>;
...
export const POST = async (req: Request) => {
  ...
  const additionalValidationResponse = validatePropertyStatus(body);

  if (additionalValidationResponse.status === "error") {
    return returnJSONResponse({
      status: "error",
      error: additionalValidationResponse.message,
      errorCode: 422,
    });
  }
  */
...
export const DELETE = async (req: Request) => {
  ...
  const body = (await req.json()) as PropertyStatusDeletePayload;
  const { deletedPropertyStatus } = body;

  if (deletedPropertyStatus.length > 0) {
    const { data, error } = await supabase
      .schema(AppConfig.sanitizedAppName)
      .from(modelConfig.tableName)
      .delete()
      .in(primaryKey, deletedPropertyStatus);
