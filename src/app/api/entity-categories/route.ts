...
export const GET = async (req: Request) => {
  ...
  const query = parseParams(searchParams) as Partial<EntityCategorySearchParams>;
...
export const POST = async (req: Request) => {
  ...
  const additionalValidationResponse = validateEntityCategory(body);
...
export const DELETE = async (req: Request) => {
  ...
  const body = (await req.json()) as EntityCategoryDeletePayload;
  const { deletedEntityCategories } = body;

  if (deletedEntityCategories.length > 0) {
    const { data, error } = await supabase
      .schema(AppConfig.sanitizedAppName)
      .from(modelConfig.tableName)
      .delete()
      .in(primaryKey, deletedEntityCategories);
