...
export const GET = async (req: Request) => {
  ...
  const query = parseParams(searchParams) as Partial<PropertySearchParams>;
...
export const DELETE = async (req: Request) => {
  ...
  const body = (await req.json()) as PropertyDeletePayload;
  const { deletedProperties } = body;
  ...
  if (deletedProperties.length > 0) {
    const { data, error } = await supabase
      ...
      .delete()
      .in(primaryKey, deletedProperties);
