...
export const GET = async (req: Request) => {
  ...
  const query = parseParams(searchParams) as Partial<ContactCategorySearchParams>;
...
export const POST = async (req: Request) => {
  ...
  const additionalValidationResponse = validateContactCategory(body);
...
export const DELETE = async (req: Request) => {
  ...
  const body = (await req.json()) as ContactCategoryDeletePayload;
  const { deletedContactCategories } = body;

  if (deletedContactCategories.length > 0) {
    const { data, error } = await supabase
      .schema(AppConfig.sanitizedAppName)
      .from(modelConfig.tableName)
      .delete()
      .in(primaryKey, deletedContactCategories);
