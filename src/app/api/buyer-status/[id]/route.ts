...
export const PUT = async (
  ...
  */
  ...
  const { id, ...relatedModels } = data;
  const relatedModelKeys = Object.keys(relatedModels).reduce((keys, key) => {
    const model = relatedModels[key];
    if (
      Object.keys(model.updated).length > 0 ||
      Object.keys(model.inserted).length > 0
    ) {
      keys.push(key);
    }
    return keys;
  ...
  return NextResponse.json({
    ...
    modelsToRequery: relatedModelKeys,
    ...newRecords,
  });
};
