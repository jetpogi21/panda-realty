...
export const GET = async (req: Request) => {
  ...
  const query = parseParams(searchParams) as Partial<BuyerStatusSearchParams>;
...
export const POST = async (req: Request) => {
  ...
  const additionalValidationResponse = validateBuyerStatus(body);
...
export const DELETE = async (req: Request) => {
  ...
  const body = (await req.json()) as BuyerStatusDeletePayload;
  const { deletedBuyerStatus } = body;

  if (deletedBuyerStatus.length > 0) {
    const { data, error } = await supabase
      .schema(AppConfig.sanitizedAppName)
      .from(modelConfig.tableName)
      .delete()
      .in(primaryKey, deletedBuyerStatus);
