...
export const findLeftForeignKeyField = (seqModelRelationshipID: number) => {
  const relationship = findConfigItemObject(
...
export const findFielToBeInsertedField = (seqModelRelationshipID: number) => {
  const relationship = findConfigItemObject(
    AppConfig.relationships,
    "seqModelRelationshipID",
    seqModelRelationshipID
...
export const findRelationshipModelConfig = (
  seqModelRelationshipID: number,
  type: "LEFT" | "RIGHT" | "TROUGH"
) => {
  const relationship = findConfigItemObject(
    AppConfig.relationships,
    "seqModelRelationshipID",
  ...
  if (type === "TROUGH" && !relationship.through) {
    throw new Error(
      "Cannot process through relationship because 'through' property of the relationship is null"
    );
  }
